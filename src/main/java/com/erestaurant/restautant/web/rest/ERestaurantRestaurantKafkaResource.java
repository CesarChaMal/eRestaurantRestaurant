...
import com.erestaurant.restautant.config.KafkaSseConsumer;
import com.erestaurant.restautant.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/e-restaurant-restaurant-kafka")
public class ERestaurantRestaurantKafkaResource {

    private final Logger log = LoggerFactory.getLogger(ERestaurantRestaurantKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public ERestaurantRestaurantKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
